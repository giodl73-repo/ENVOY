...
fn run(args: &[String]) -> Result<String, String> {
    let [command, path] = args else {
        return Err(
            "usage: envoy <analyze|held-pack|official-baseline|official-held-pack|candidate-baseline|candidate-held-pack|level2-baseline|level2-held-pack|evaluation-readiness|evaluation-held-pack|safeguard-baseline|safeguard-held-pack|outcome-reference|outcome-held-pack|result-chain-audit|result-chain-held-pack|program-scenarios|program-realization|program-accounting|program-alternatives|program-incidence|program-delivery|program-adaptive|program-peers|program-held-pack> <fixture.tsv>"
                .into(),
        );
    };
    ...
    if command.starts_with("program-") {
        return envoy_program::run(command, &input);
    }
    if command.starts_with("candidate-")
        ...
        || command.starts_with("result-chain-")
    {
